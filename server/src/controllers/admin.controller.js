...
export const createVariable = async (req, res) => {
  try {
    ...
    if (!variableName || !variableKey) {
      return res
        .status(400)
        .json({ error: "variableName and variableKey are required" });
    }

    // Check for duplicates
    const existingVariable = await TemplateVariable.findOne({
      where: {
        [Sequelize.Op.or]: [{ variableName }, { variableKey }],
      },
    });
...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    // Email stats
    const totalSentEmails = await SentEmail.count({
      where: { status: "sent" },
    });
    ...
    // File upload stats
    const totalUploads = await UploadedRow.count({
      distinct: true,
      col: "file_id",
    });
    ...
    const activeJobs = await ScheduledEmail.count({
      where: { status: "in_progress" },
    });
    const completedJobs = await ScheduledEmail.count({
