...
export const replacePlaceholders = async (text, row) => {
  ...
  processedText = processedText.replace(
    ...
    (match, variableName) => {
      ...
      if (
        excelData[trimmedName] !== undefined &&
        excelData[trimmedName] !== null &&
        excelData[trimmedName] !== ""
      ) {
        value = excelData[trimmedName];
        foundKey = trimmedName;
      } else if (
        row[trimmedName] !== undefined &&
        row[trimmedName] !== null &&
        row[trimmedName] !== ""
      ) {
      ...
      if (trimmedName === "Calendar Link" && value) {
        ...
        return `<a href="${linkUrl}" target="_blank" rel="noopener noreferrer" style="color: #1a73e8; text-decoration: underline;">Calendar Link</a>`;
      }
